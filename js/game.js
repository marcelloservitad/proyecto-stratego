...
function initializeGame() {
    ...
    let pieceCount = 40; // Valor por defecto
    
    if (deploymentData) {
        try {
            const deployment = JSON.parse(deploymentData);
            pieceCount = deployment.pieces ? deployment.pieces.length : 40;
        } catch (e) {
            console.error('Error al parsear deployment data:', e);
        }
    }
    
    console.log(`Juego inicializado - ${pieceCount} piezas por ejército`);
            ...
            if (piece.position) {
                ...
                if (row >= 6 && row < 10 && col >= 0 && col < 10 && !isLakePosition(row, col)) {
                    ...
                    placePiece(row, col, {
                        ...
                        player: 'player',
                        revealed: true,
                        symbol: armyUnit?.symbol || ':question:'
                    });
                }
            }
        });
        
        // CORRECCIÓN: Usar el conteo real de piezas del jugador en lugar de 40 fijo
...
function createOpponentArmy(gameType) {
    console.log(`Creando ejército del oponente (modalidad: ${ localStorage.getItem('mode')})...`);
    const army = generateFullArmy('opponent', localStorage.getItem('mode'));
    let placed = 0;
